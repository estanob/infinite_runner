...
class Game {
  constructor(ctx) {
    ...
    this.numIcebergs = Math.floor(Math.random() * (1000 - 50 + 1) + 50); // Min: 50, Max: 1000
    this.icebergsHit = 0;
    this.ctx = ctx;
  }
  ...
  addIcebergs() {
    for (let i = 0; i < this.numIcebergs; i++) {
      if (this.icebergsPassed < this.numIcebergs) {
        if (this.numIcebergs - this.icebergsPassed >= 8 && this.icebergs.length < 8) {
          this.icebergs.push(
            new Iceberg({
              game: this,
  ...
  powerUp() {
    if (this.numIcebergs <= 150) {
      ...
      if ((this.icebergsHit >= (this.numIcebergs * .1)) && (this.icebergsHit < (this.numIcebergs * .19))) {
        this.powerDistance = 200;
      ...
      if ((this.icebergsHit >= (this.numIcebergs * .085)) && (this.icebergsHit < (this.numIcebergs * .15))) {
        this.powerDistance = 200;
